...
CARMA<-function(z.list,ld.list,w.list=NULL,lambda.list,
  ...
  ########## Input data#########
  {
    ...
    if(!is.null(w.list)){
      ...
      w.list[[i]][,is.nan(colSums(w.list[[i]]))]<-0
      }
    }
    Sigma.list<-list()
    Sigma.inv.list<-list()
      ...
      ##############COMPUTATION ############
        {
          ...
          if(length(S)!=0){
           ...
           print(set.star)
            ...
            set.star$gamma.set.index <-c(sample((1:length(set.gamma.margin[[2]]))[order(exp(aa),decreasing = T)[1:(floor(p/2))]],
                                                1,prob=exp(aa)[order(exp(aa),decreasing = T)[1:(floor(p/2))]]))
            set.star$margin<-set.gamma.margin[[2]][  set.star$gamma.set.index]
    ...
    if(class(try.index)[1]!='try-error'){
     ...
     if(is.null(input.prior.prob)){
      ...
      for(i in 1:L){
        ...
        prior.prob.list<-input.prior.prob
      }
    }
    ########    
