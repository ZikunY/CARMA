...
CARMA_fixed_sigma_SG<-function(z.list,ld.list,w.list=NULL,lambda.list=NULL,output.labels=NULL,label.list=NULL,
...
#########Module model##########
 ...
 #     z<-as.matrix(pro.data$Z);B=2e+4;Sigma<-ld;input.S=NULL
  #    lambda<-1;y.var=1;num.causal=10
        ...
        { 
          ...
          if(length(working.S)!=0){
            ...
            for(i in 2:length(set.gamma)){
              ...
              set.star$margin[i]<-set.gamma.margin[[i]][  set.star$gamma.set.index[i]]
              set.gamma[[2]][set.star$gamma.set.index[2],]
