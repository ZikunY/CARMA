...
double marginal_fun_indi(const double &zSigmaz_S, const double &tau, 
...
// [[Rcpp::export]]
double Cauchy_fixed_sigma_marginal(const arma::uvec & index_vec_input, const arma::mat & Sigma,
  ...
  for(unsigned int j=0;j<S;j++){
    b[j]=marginal_fun_indi(zSigmaz_S(0,0),tau[j],p_S,y_sigma);
  }
  double results=log(mean(b))+zSigmaz_S(0,0)/2.00;
